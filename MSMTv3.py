...
false_string = " = false"
censor_string = "    \"faggot\", \"fellatio\", \"fuck\", \"gas the \", \"Hitler\", \"jizz\", \"nigga\", \"nigger\", " \
                "\"smegma\", \"vagina\", \"whore\" }"
...
config_dict = {"passTimeWhenEmpty =": '', "allowConsole =": '', "allowBedRest =": '', "allowWildernessRest =": '',
               ...
               "deathTime =": '', "deathPenaltyJailDays =": '', "disallowedNameStrings =": '',
               "ignoreScriptErrors =": '', "useActorCollisionForPlacedObjects =": '',
               "enablePlacedObjectCollision =": '', "enableActorCollision =": '', "bannedEquipmentItems =": '',
               "ignoreModifierWithMaxSkill =": '', "physicsFramerate =": '', "pingDifferenceRequiredForAuthority =": '',
               "fixmeInterval =": '', "maxAcrobaticsValue =": '', "maxSkillValue =": '', "maxSpeedValue =": '',
               "maxAttributeValue =": '', "useInstancedSpawn =": '', "maximumObjectScale =": '',
               "allowOnContainerForUnloadedCells =": ''}  # dic for config.lua
...
class Ui(QtWidgets.QMainWindow):  # class for GUI
    ...
    def __init__(self):  # initialise GUI function
        ...
        self.ignoreScriptErrors = self.findChild(QtWidgets.QCheckBox, 'ignoreScriptErrors')  # Find the checkbox
        if str(config_dict["ignoreScriptErrors ="]) == "true":  # Read value
            self.ignoreScriptErrors.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["ignoreScriptErrors ="]) == "false":  # Read value
            self.ignoreScriptErrors.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.ignoreScriptErrors.stateChanged.connect(self.boolean_config_change)  # call function

        self.useActorCollisionForPlacedObjects = self.findChild \
            (QtWidgets.QCheckBox, 'useActorCollisionForPlacedObjects')  # Find the checkbox
        if str(config_dict["useActorCollisionForPlacedObjects ="]) == "true":  # Read value
            self.useActorCollisionForPlacedObjects.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["useActorCollisionForPlacedObjects ="]) == "false":  # Read value
            self.useActorCollisionForPlacedObjects.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.useActorCollisionForPlacedObjects.stateChanged.connect(self.boolean_config_change)  # call function

        self.enablePlacedObjectCollision = self.findChild \
            (QtWidgets.QCheckBox, 'enablePlacedObjectCollision')  # Find the checkbox
        if str(config_dict["enablePlacedObjectCollision ="]) == "true":  # Read value
            self.enablePlacedObjectCollision.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["enablePlacedObjectCollision ="]) == "false":  # Read value
            self.enablePlacedObjectCollision.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.enablePlacedObjectCollision.stateChanged.connect(self.boolean_config_change)  # call function

        self.enableActorCollision = self.findChild(QtWidgets.QCheckBox, 'enableActorCollision')  # Find the checkbox
        if str(config_dict["enableActorCollision ="]) == "true":  # Read value
            self.enableActorCollision.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["enableActorCollision ="]) == "false":  # Read value
            self.enableActorCollision.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.enableActorCollision.stateChanged.connect(self.boolean_config_change)  # call function

        self.ignoreModifierWithMaxSkill = self.findChild \
            (QtWidgets.QCheckBox, 'ignoreModifierWithMaxSkill')  # Find the checkbox
        if str(config_dict["ignoreModifierWithMaxSkill ="]) == "true":  # Read value
            self.ignoreModifierWithMaxSkill.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["ignoreModifierWithMaxSkill ="]) == "false":  # Read value
            self.ignoreModifierWithMaxSkill.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.ignoreModifierWithMaxSkill.stateChanged.connect(self.boolean_config_change)  # call function

        self.allowOnContainerForUnloadedCells = self.findChild \
            (QtWidgets.QCheckBox, 'allowOnContainerForUnloadedCells')  # Find the checkbox
        if str(config_dict["allowOnContainerForUnloadedCells ="]) == "true":  # Read value
            self.allowOnContainerForUnloadedCells.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["allowOnContainerForUnloadedCells ="]) == "false":  # Read value
            self.allowOnContainerForUnloadedCells.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.allowOnContainerForUnloadedCells.stateChanged.connect(self.boolean_config_change)  # call function

        self.useInstancedSpawn = self.findChild(QtWidgets.QCheckBox, 'useInstancedSpawn')  # Find the checkbox
        if str(config_dict["useInstancedSpawn ="]) == "true":  # Read value
            self.useInstancedSpawn.setCheckState(Qt.CheckState.Checked)  # check box
        if str(config_dict["useInstancedSpawn ="]) == "false":  # Read value
            self.useInstancedSpawn.setCheckState(Qt.CheckState.Unchecked)  # uncheck box
        self.useInstancedSpawn.stateChanged.connect(self.boolean_config_change)  # call function
        ...
        self.deathPenaltyView = self.findChild(QtWidgets.QTextBrowser, 'deathPenaltyView')  # Find the text browser
        ...
        self.disallowedNameStrings = self.findChild(QtWidgets.QPushButton, 'disallowedNameStrings')  # Find the button
        self.disallowedNameStrings.clicked.connect(self.censor_param_config_change)  # Connect button to a function

        self.bannedEquipmentItems = self.findChild(QtWidgets.QPushButton, 'bannedEquipmentItems')  # Find the button
        self.bannedEquipmentItems.clicked.connect(self.censor_param_config_change)  # Connect button to a function
        ...
        self.physicsFramerateView = self.findChild \
            (QtWidgets.QTextBrowser, 'physicsFramerateView')  # Find the text browser
        self.physicsFramerateView.setHtml(html_center_string + config_dict["physicsFramerate ="] +
                                          html_end_string)  # Show the content of variable in html form

        self.physicsFramerate = self.findChild(QtWidgets.QPushButton, 'physicsFramerate')  # Find the button
        self.physicsFramerate.clicked.connect(self.param_config_change)  # Connect button to a function

        self.pingDifferenceRequiredForAuthorityView = self.findChild \
            (QtWidgets.QTextBrowser, 'pingDifferenceRequiredForAuthorityView')  # Find the text browser
        self.pingDifferenceRequiredForAuthorityView.setHtml \
            (html_center_string + config_dict["pingDifferenceRequiredForAuthority ="] +
             html_end_string)  # Show the content of variable in html form

        self.pingDifferenceRequiredForAuthority = self.findChild \
            (QtWidgets.QPushButton, 'pingDifferenceRequiredForAuthority')  # Find the button
        self.pingDifferenceRequiredForAuthority.clicked.connect \
            (self.param_config_change)  # Connect button to a function

        self.fixmeIntervalView = self.findChild(QtWidgets.QTextBrowser, 'fixmeIntervalView')  # Find the text browser
        self.fixmeIntervalView.setHtml(html_center_string + config_dict["fixmeInterval ="] +
                                       html_end_string)  # Show the content of variable in html form

        self.fixmeInterval = self.findChild(QtWidgets.QPushButton, 'fixmeInterval')  # Find the button
        self.fixmeInterval.clicked.connect(self.param_config_change)  # Connect button to a function

        self.maxAcrobaticsValueView = self.findChild \
            (QtWidgets.QTextBrowser, 'maxAcrobaticsValueView')  # Find the text browser
        self.maxAcrobaticsValueView.setHtml(html_center_string + config_dict["maxAcrobaticsValue ="] +
                                            html_end_string)  # Show the content of variable in html form

        self.maxAcrobaticsValue = self.findChild(QtWidgets.QPushButton, 'maxAcrobaticsValue')  # Find the button
        self.maxAcrobaticsValue.clicked.connect(self.param_config_change)  # Connect button to a function

        self.maxSkillValueView = self.findChild(QtWidgets.QTextBrowser, 'maxSkillValueView')  # Find the text browser
        self.maxSkillValueView.setHtml(html_center_string + config_dict["maxSkillValue ="] +
                                       html_end_string)  # Show the content of variable in html form

        self.maxSkillValue = self.findChild(QtWidgets.QPushButton, 'maxSkillValue')  # Find the button
        self.maxSkillValue.clicked.connect(self.param_config_change)  # Connect button to a function

        self.maxSpeedValueView = self.findChild(QtWidgets.QTextBrowser, 'maxSpeedValueView')  # Find the text browser
        self.maxSpeedValueView.setHtml(html_center_string + config_dict["maxSpeedValue ="] +
                                       html_end_string)  # Show the content of variable in html form

        self.maxSpeedValue = self.findChild(QtWidgets.QPushButton, 'maxSpeedValue')  # Find the button
        self.maxSpeedValue.clicked.connect(self.param_config_change)  # Connect button to a function

        self.maxAttributeValueView = self.findChild(QtWidgets.QTextBrowser,
                                                    'maxAttributeValueView')  # Find the text browser
        self.maxAttributeValueView.setHtml(html_center_string + config_dict["maxAttributeValue ="] +
                                           html_end_string)  # Show the content of variable in html form

        self.maxAttributeValue = self.findChild(QtWidgets.QPushButton, 'maxAttributeValue')  # Find the button
        self.maxAttributeValue.clicked.connect(self.param_config_change)  # Connect button to a function

        self.maximumObjectScaleView = self.findChild(QtWidgets.QTextBrowser,
                                                     'maximumObjectScaleView')  # Find the text browser
        self.maximumObjectScaleView.setHtml(html_center_string + config_dict["maximumObjectScale ="] +
                                            html_end_string)  # Show the content of variable in html form

        self.maximumObjectScale = self.findChild(QtWidgets.QPushButton, 'maximumObjectScale')  # Find the button
        self.maximumObjectScale.clicked.connect(self.param_config_change)  # Connect button to a function
    ...
    def censor_param_config_change(self):  # Universal parameter function for config file
        signal_sender = self.sender()  # Assign sender to a var
        button_name = (signal_sender.objectName())  # Get the name of sender button
        caller = "" + str(button_name) + " ="  # Assign the name of sender button as a string with =
        param_config_content = open(config_dir, 'r')  # open config file with read permissions
        ...
        replacement = ""  # empty string for replacement purposes
        for config_line in param_config_content:  # listing for function and assign a replacement line to a var.
            changes = config_line.replace(censor_string, "")
            replacement = replacement + changes  # merge string
        param_config_content.close()  # close config file
        file_out = open(config_dir, "w")  # open config file with write permissions
        file_out.write(replacement)  # write replacement
    ...
    def banned_param_config_change(self):  # Universal parameter function for config file
        signal_sender = self.sender()  # Assign sender to a var
        button_name = (signal_sender.objectName())  # Get the name of sender button
        caller = "" + str(button_name) + " ="  # Assign the name of sender button as a string with =
        param_config_content = open(config_dir, 'r')  # open config file with read permissions
